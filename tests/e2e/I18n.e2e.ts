...
test.describe('I18n', () => {
  test.describe('Language Switching', () => {
    test('should switch language from English to French using dropdown and verify text on the homepage', async ({ page }) => {
      ...
      await expect(page.getByText('The perfect microlearning platform to train')).toBeVisible();

      await page.getByRole('button', { name: 'Change language' }).click();
      await page.getByText('Français').click();

      await expect(page.getByText('La plateforme de microlearning parfaite pour former')).toBeVisible();
    });
